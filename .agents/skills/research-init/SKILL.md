...
Before writing, read `AGENTS.md`, `docs/RESEARCH_WORKFLOW.md`, `research_data/PROJECT.md` (when present), and `research_data/STATE.md` (when present). If `research_data/graph.json` exists, stop: initialization never overwrites an existing project.

Create the interview result as a temporary JSON input outside `research_data/`. Before any project write:
...
2. Run `node scripts/research-cli.mjs preview --input <file>` and summarize the proposed objectives, aspects, questions, timeline, and team.
3. Ask for explicit approval.
4. Only after approval run `node scripts/research-cli.mjs init --input <file>`.
5. Run `node scripts/research-cli.mjs state`; a stale result is an error.
...
Do not modify `config.json`. Finish by pointing the researcher to `research_data/PROJECT.md`, `research_data/STATE.md`, and `npm run dev`.
